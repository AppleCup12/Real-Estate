              ...
              <nav class="nav-bar-list">
                <div class="padding-list-nav">
                  ...
                  <div class="navbar-nav w-100">
                    ...
                    <a href="../confirm-real/confirm-real" class="nav-item nav-link active" style="position: relative;">&nbsp;<i class="bi bi-card-checklist"></i> ຄວາມຄິດເຫັນ&nbsp;
                      <label class="sms-show" style="font-size: 14px;background-color: red;color: #ffff; border-radius: 100%; width: 26px; height: 26px; text-align: center; position: absolute; padding: 3px">
                        <?PHP 
        ...
        <div class="col-md-9">
          <div class="page-banner banner-about">
            <div class="group-padding">
              <div class="group">
                  <div class="header"><h3><i class="bi bi-card-checklist"></i> ຄວາມຄິດເຫັນຂອງຜູ້ໃຊ້</h3></div>
                  <hr>
              </div>
              <div class="group-nav">
                <nav>
                  <a href="../dashboard/dashboard" id="a1">ໜ້າແລກ</a> |
                  <a href="" id="a2">ຂໍຄວາມ</a> |
                  <a href="#:+-+:LOVE" id="a3">ຄວາມຄິດເຫັນ</a> 
                </nav>
              </div>
              &nbsp;
              <div class="group-list-user">
                <div class="text-user">
                    <h4 id="text-user">ຄວາມຄິດເຫັນຂອງຜູ້ໃຊ້ ທີ່ຄວນນຳໄປປັບປຸງ</h4>
                    <p id="text-about">ກະລຸນາກວດສອບຄວາມຄິດເຫັນທີ່ເໝາະສົມ</p>
                  </div>
              </div>
              <div class="card">
                <?PHP 
                  $comment = mysqli_query ($conn, "SELECT * FROM `comment` ORDER BY cm_id DESC");
                  $one = 1;
                  foreach($comment as $row_comment){
                    $uid_user = $row_comment['cm_id'];
                    $user = mysqli_fetch_array (mysqli_query($conn, "SELECT * FROM `users` WHERE `user_id`=$uid_user"));
                ?>
                <div class="group-comment">
                  <div class="row">
                    <div class="column-one"><b><?PHP echo $one++ ?></b></div>
                    <div class="column-text"><?PHP echo $row_comment['cm_name'] ?></div>
                    <div class="column-user"><img src="../../document/profile/upload/<?PHP echo $user['user_img'] ?>" alt="ບໍ່ມີຮູບ" style="width: 50px; height: 50px; border-radius: 100%"></div>
                  </div>
                </div>
                <div class="wiht"></div>
                <?PHP } ?>
